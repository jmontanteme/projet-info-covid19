...
metadata_path = '/Users/juliettemontanteme/Desktop/metadata_processed.csv'
...
scores = np.zeros((len(meta_df), 13))
...
for i in range(13):
    ...
    for j in range(len(meta_df)):
        if j%1000 == 0:
           print(j) 
        path = meta_df['path'].iloc[j] 
        try: 
            sc = Scoring(path, lex, lex_name)
            sc.matching()
            sc.scoring()
            scores[j, i] = sc.score_dif
            scores[j, i+1] = sc.score_fr_title
            scores[j, i+2] = sc.score_fr_abstract
            scores[j, i+3] = sc.score_fr_doc
        except(FileNotFoundError):
            pass
    print(f'{lex_name} finished')
